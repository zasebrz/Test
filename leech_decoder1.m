...
for k=1:data_length
    ...
    [min_a1,index11]=min(d_a2,[],2);
    [min_b,index2]=min(d_b1,[],2);
    [min_b1,index22]=min(d_b2,[],2);
    for j=1:12
        switch index1(j)
            case 1
   ...
   if(mod(sum(a),2)==0)
       ...
       for j=1:12
           d_aerror(j)=d_a2(j,index11(j));
       end
       [d_aerrormin,aerror_index]=min(d_aerror);
       a(aerror_index)=mod(a(aerror_index)+1,2);
   ...
   if(mod(sum(b),2)==1)
       ...
       for j=1:12
           d_berror(j)=d_b2(j,index22(j));
       end
       [d_berrormin,berror_index]=min(d_berror);
       b(berror_index)=mod(b(berror_index)+1,2);
